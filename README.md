...
python3 scripts/package_skill.py --smoke-test
python3 scripts/package_skill.py --clean
```

这些命令会检查 skill 包结构、多 Agent 兼容说明，执行临时打包 smoke test，并确保运行时日志、反馈记录和本地缓存不会进入发布包。
...
`improve_once.py` 还会在每轮运行目录下生成一份普通用户更容易阅读的报告：

```text
evaluation/runs/<run-name>/user_report.md
```

这份报告会说明本轮测试了什么、还缺哪些输出、当前分数信号、主要失败标签和建议下一步。
...
当你认可策略框架后，可以继续让 Skill 输出适合下游工具的 brief。当前支持通用 AI PPT brief，也提供 Gamma、Canva、Marp、归藏 PPT、宝玉 slide deck 等适配建议。

示例：

```text
把这个框架整理成给 Gamma 的生成 brief。缺少的数据不要编，用 [placeholder] 保留。
```

```text
把这份销售 deck 框架改成适合宝玉 slide deck 的 outline 和图片生成提示方向。
```

相关规则在：

```text
presentation-strategist/references/handoff-adapters.md
```
